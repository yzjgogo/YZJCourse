...
/**
 * y由[K1泛型擦除]可以，泛型类型在运行时是被擦除的，你无法知道运行时一个类引用了什么类型的泛型，函数也是一样；
 *
 * 什么是实化类型参数？真的实化了吗？
 * 所谓实化类型参数，就是让泛型类或泛型函数在运行时保留泛型的类型实参，看起来就像没有擦除一样，
...
//    fun <T> isA(value: Any) = value is T
    ...
    //当其它函数调用isA()是T会被替换成具体的类型，再把isA()的整套代码复制到调用出，巧妙的避开了泛型
