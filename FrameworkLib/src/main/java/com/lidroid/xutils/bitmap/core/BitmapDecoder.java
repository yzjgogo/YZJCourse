...
public class BitmapDecoder {
    ...
    public static Bitmap decodeSampledBitmapFromByteArray(byte[] data, BitmapSize maxSize, BitmapDisplayConfig config) {
        synchronized (lock) {
            final BitmapFactory.Options options = new BitmapFactory.Options();
            options.inJustDecodeBounds = true;
            options.inPurgeable = true;
            options.inInputShareable = true;
            options.inTargetDensity = config.getTargetDensity();
            options.inDensity = config.getBitmapDensity();
            options.inScaled = true;
            BitmapFactory.decodeByteArray(data, 0, data.length, options);
            options.inSampleSize = calculateInSampleSize(options, maxSize.getWidth(), maxSize.getHeight());
            options.inJustDecodeBounds = false;
            if (config != null && config.getBitmapConfig() != null) {
                options.inPreferredConfig = config.getBitmapConfig();
            }
            try {
